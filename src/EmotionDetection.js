...
function EmotionDetection() {
    ...
    const predictEmotions = async () => {
        setLoading(true);
        try {
            const response = await fetch('https://aldonast1907-emocarebackend.hf.space/predict', {
                method: 'POST',
    ...
    const getSuggestions = async (emotion) => {
        ...
        try {
            const response = await fetch(`https://aldonast1907-emocarebackend.hf.space/suggestions?emotion=${emotion}&text=${encodeURIComponent(text)}`);
